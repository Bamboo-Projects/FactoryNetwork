...
| 5.6 | **Der Connector im Kabelblock**, wie bei AE2. Gewünscht am 26.08. **Entwurf steht:** `connector-im-kabel.md` — die Frage ist in Wahrheit zwei. **A:** Der Connector verbindet sich wie ein Kabel; ein Block je Maschine statt zwei, und **keine** der 106 Stellen, die `ConnectorBlockEntity` über `getBlockEntity(pos)` holen, wird angefasst. **B:** echtes Multipart mit sechs Teilen je Block — dann jede einzelne davon, dazu Redstone, Benennen und die Blickrichtung im BlockState. Empfehlung ist A; B ist **jetzt billiger als je wieder**, weil bestehende Welten Prüfstände sind | E | `connector-im-kabel.md` | A klein, B groß | A oder B |
